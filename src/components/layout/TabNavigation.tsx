...
const TAB_ITEMS: NavTab[] = [
  ...
  { label: 'SUPER ADMIN', path: '/admin', roles: [UserRole.ADMIN, UserRole.SUPER_ADMIN], icon: <Shield size={13} /> },
]
...
export default function TabNavigation() {
  ...
  const isActive = (path: string) => {
    ...
    // Super Admin tab active for all admin sub-pages
    if (path === '/admin') {
      return ['/admin', '/organizations', '/roles', '/audit-log', '/api-diagnostics', '/api-docs', '/sdk-examples', '/test-environment'].some(
