...
export default function MapPage() {
    ...
    // MAPBOX_TOKEN moved to env variable
    ...
    return (_jsxs("div", { className: "flex h-[calc(100vh-80px)] gap-4", children: [_jsxs("div", { className: `relative ${isFullscreen ? 'w-full' : 'flex-1'} rounded-lg border border-gray-200 overflow-hidden shadow-sm`, children: [_jsxs(MapContainer, { center: [43.7, 3.87], zoom: 6, className: "h-full w-full z-0", zoomControl: false, children: [_jsx(TileLayer, { url: tileUrl, attribution: tileAttribution, tileSize: 512, zoomOffset: -1 }), _jsx(FitBounds, { vehicles: vehicles }), selectedVehicle?.currentLat && selectedVehicle?.currentLng && (_jsx(FlyToVehicle, { lat: selectedVehicle.currentLat, lng: selectedVehicle.currentLng })), vehiclesWithGps.map((vehicle) => (_jsx(Marker, { position: [vehicle.currentLat, vehicle.currentLng], icon: createVehicleIcon(vehicle.currentSpeed || 0, vehicle.currentHeading || 0, vehicle.id === selectedVehicleId, vehicle.type), eventHandlers: {
                                ...
                                }, className: "gap-2 bg-white shadow-md", children: [tileLayer === 'streets' ? _jsx(Satellite, { size: 16 }) : tileLayer === 'satellite' ? _jsx(MapIcon, { size: 16 }) : _jsx(Layers, { size: 16 }), tileLayer === 'streets' ? 'Satellite' : tileLayer === 'satellite' ? 'Terrain' : 'Plan'] }), _jsxs(Button, { variant: "outline", size: "sm", onClick: () => setIsFullscreen(!isFullscreen), className: "gap-2 bg-white shadow-md", children: [_jsx(Navigation, { size: 16 }), isFullscreen ? 'Quitter plein écran' : 'Plein écran'] })] }), _jsxs("div", { className: "absolute bottom-4 left-4 z-[1000] flex gap-2", children: [_jsxs("div", { className: "rounded-full bg-white px-3 py-1 shadow-md text-xs font-medium flex items-center gap-1.5", children: [_jsx("span", { className: "h-2.5 w-2.5 rounded-full bg-green-500 inline-block" }), movingCount, " en mouvement"] }), _jsxs("div", { className: "rounded-full bg-white px-3 py-1 shadow-md text-xs font-medium flex items-center gap-1.5", children: [_jsx("span", { className: "h-2.5 w-2.5 rounded-full bg-gray-400 inline-block" }), stoppedCount, " \u00E0 l'arr\u00EAt"] }), _jsxs("div", { className: "rounded-full bg-white px-3 py-1 shadow-md text-xs font-medium", children: [vehiclesWithGps.length, " / ", vehicles.length, " GPS actif"] }), _jsxs("div", { className: `rounded-full px-3 py-1 shadow-md text-xs font-medium flex items-center gap-1.5 ${isConnected ? 'bg-green-50 text-green-700' : 'bg-white text-gray-500'}`, children: [isConnected ? _jsx(Wifi, { size: 12 }) : _jsx(WifiOff, { size: 12 }), isConnected ? 'Live' : 'Polling'] })] })] }), !isFullscreen && (_jsxs("div", { className: "w-80 flex flex-col gap-4 overflow-hidden", children: [_jsx(Card, { children: _jsx(CardContent, { className: "pt-4 pb-4", children: _jsxs("div", { className: "relative", children: [_jsx(Search, { className: "absolute left-3 top-3 text-gray-400", size: 16 }), _jsx(Input, { type: "search", placeholder: "Rechercher un v\u00E9hicule...", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), className: "pl-9 h-10" })] }) }) }), _jsxs(Card, { className: "flex-1 overflow-hidden flex flex-col", children: [_jsx(CardHeader, { className: "pb-2 pt-4", children: _jsxs(CardTitle, { className: "text-sm font-semibold text-gray-700", children: ["V\u00E9hicules (", filteredVehicles.length, ")"] }) }), _jsx(CardContent, { className: "flex-1 overflow-y-auto pb-4", children: _jsx("div", { className: "space-y-1.5", children: filteredVehicles.map((vehicle) => {
